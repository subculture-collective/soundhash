...
def benchmark_fingerprint_extraction():
    ...
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as tmp:
        ...
        try:
            _fingerprint_data = fingerprinter.extract_fingerprint(tmp.name)
...
def benchmark_fingerprint_comparison():
    ...
    try:
        ...
        fp2 = (
            fp1 + np.random.rand(sample_size).astype(np.float32) * 0.1
        ...
        for _ in range(iterations):
            ...
            # Average similarity
            _similarity = (abs(correlation) + euclidean_sim) / 2.0
...
def generate_markdown_report(results):
    ...
    for key, current_value in results.items():
        ...
        if current_value < 0:
            report.append(
                f"| {key.replace('_', ' ').title()} | FAILED | {baseline_value:.2f}ms | - | ❌ |"
            )
            continue
