...
async def main():
    ...
    parser.add_argument(
        "--skip-processing",
        action="store_true",
        help="Ingest metadata but skip processing videos",
    )
    parser.add_argument(
        "--only-process",
        action="store_true",
    ...
    logger.log_section_start(
        "SoundHash Channel Ingestion",
        "Processing YouTube channels for audio fingerprinting",
    )
    logger.info(f"🎯 Target channels: {channels}")
    logger.info(
        f"📊 Max videos per channel: {args.max_videos if args.max_videos else 'unlimited'}"
    )
    ...
    try:
        ...
        logger.log_step(
            1, "Initializing components", "Setting up ingester and video processor"
        )
        ...
        if not args.only_process:
            ...
            logger.log_step(
                ...
                "Fetching metadata and creating video records",
            )
        ...
        if not args.dry_run and not args.skip_processing:
            ...
            logger.log_step(
                3, "Video Processing", "Downloading audio and generating fingerprints"
            )
