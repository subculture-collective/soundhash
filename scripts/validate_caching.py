...
def validate_repository():
    ...
    assert (
        "check_fingerprints_exist" in repo_content
    ), "Repository should have check_fingerprints_exist method"
...
def validate_ingestion():
    ...
    assert (
        "check_fingerprints_exist" in ingestion_content
    ), "Ingestion should call check_fingerprints_exist"
...
def validate_migration():
    ...
    for migration_file in migration_files:
        ...
        if (
            "n_fft" in migration_content
            and "hop_length" in migration_content
            and "audio_fingerprints" in migration_content
        ):
            found_migration = True
            print(f"  ✓ Found migration file: {migration_file.name}")
            assert (
                "add_column" in migration_content.lower() or "Column" in migration_content
    ...
    assert (
        found_migration
    ), "Should have migration that adds n_fft and hop_length to audio_fingerprints"
...
def validate_documentation():
    ...
    assert (
        "YT_DLP_CACHE_DIR" in readme_content or "yt-dlp" in readme_content.lower()
    ), "README should mention yt-dlp cache"
