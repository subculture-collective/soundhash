...
class ProgressLogger:
    ...
    def update(self, increment=1, item_name=None):
        ...
        if self.current_item > 0:
            eta_seconds = (elapsed.total_seconds() / self.current_item) * (self.total_items - self.current_item)
        ...
        self.logger.info(f"📊 {self.description}: [{bar}] {percentage:.1f}% ({self.current_item}/{self.total_items}){item_info} | {eta}")
