...
class TestChannelIngester:
    ...
    async def test_ingest_channel_respects_max_videos(self, ingester):
        ...
        test_videos = [
            {"id": f"video{i}", "title": f"Test Video {i}", "duration": 100} for i in range(20)
        ]
    ...
    async def test_idempotent_job_creation(self, ingester):
        ...
        # Simulate job already exists (create_job_if_not_exists returns None)
        mock_job_repo.create_job_if_not_exists.return_value = None
        ...
        with (
            ...
            # Verify create_job_if_not_exists was called
            mock_job_repo.create_job_if_not_exists.assert_called()
            # Since it returned None, no new job was created (idempotent behavior)
    ...
    async def test_video_counting_no_double_count(self, ingester):
        ...
        # Jobs don't exist, so create_job_if_not_exists returns a mock job
        mock_job = MagicMock()
        mock_job_repo.create_job_if_not_exists.return_value = mock_job
        ...
        with (
            ...
            # Verify create_job_if_not_exists was called exactly 3 times (once per new video)
            assert mock_job_repo.create_job_if_not_exists.call_count == 3
