...
class TestAudioFingerprinter:
    ...
    def test_extract_fingerprint_happy_path(self, sine_wave_file):
        ...
        assert result['confidence_score'] >= 0.0  # Confidence score can be > 1.0 in current implementation
    ...
    def test_serialize_deserialize_fingerprint(self, sine_wave_file):
        ...
        np.testing.assert_array_almost_equal(
            original['compact_fingerprint'],
            deserialized['compact_fingerprint']
        )
        ...
        fingerprint_data = [
            {
                'time': 0.0,
                'peaks': [
                    {'frequency': 440.0, 'bin': 20, 'magnitude': 100.0, 'freq_range': 1}
                ]
            }
        ]
