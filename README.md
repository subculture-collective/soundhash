...
All configuration is managed through environment variables in the `.env` file. The application uses centralized configuration management with validation to ensure critical settings are provided.

#### Database Configuration

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `DATABASE_URL` | Conditional* | - | Full PostgreSQL connection string (e.g., `postgresql://user:pass@host:5432/db`) |
| `DATABASE_HOST` | Conditional* | `localhost` | PostgreSQL server hostname |
| `DATABASE_PORT` | No | `5432` | PostgreSQL server port |
| `DATABASE_NAME` | No | `soundhash` | Database name |
| `DATABASE_USER` | Conditional* | - | Database username |
| `DATABASE_PASSWORD` | Conditional* | - | Database password |

*Either `DATABASE_URL` or (`DATABASE_USER` and `DATABASE_PASSWORD`) must be provided.

#### Processing Configuration

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `TEMP_DIR` | No | `./temp` | Directory for temporary audio files |
| `SEGMENT_LENGTH_SECONDS` | No | `90` | Audio segment length for fingerprinting |
| `FINGERPRINT_SAMPLE_RATE` | No | `22050` | Sample rate (Hz) for audio fingerprinting |
| `MAX_CONCURRENT_DOWNLOADS` | No | `3` | Maximum parallel video downloads |
| `MAX_CONCURRENT_CHANNELS` | No | `2` | Maximum parallel channel ingestions |
| `KEEP_ORIGINAL_AUDIO` | No | `true` | Keep original downloaded audio files |
| `CLEANUP_SEGMENTS_AFTER_PROCESSING` | No | `true` | Delete audio segments after fingerprinting |

#### YouTube Download Hardening

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `USE_PROXY` | No | `false` | Enable proxy for downloads |
| `PROXY_URL` | No | - | Single proxy URL (e.g., `http://proxy.example.com:8080`) |
| `PROXY_LIST` | No | - | Comma-separated list of proxy URLs for rotation |
| `YT_COOKIES_FILE` | No | - | Path to Netscape cookies.txt file exported from browser |
| `YT_COOKIES_FROM_BROWSER` | No | - | Browser name for cookie extraction (`chrome`, `firefox`, `brave`, `edge`) |
| `YT_BROWSER_PROFILE` | No | - | Browser profile name (e.g., `Default`, `Profile 1`) |
| `YT_PLAYER_CLIENT` | No | - | Override player client (`android`, `ios`, `web_safari`, `tv`) |

#### API Keys (Optional)

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `YOUTUBE_API_KEY` | No | - | YouTube Data API v3 key (OAuth preferred) |
| `TWITTER_BEARER_TOKEN` | No | - | Twitter API v2 bearer token |
| `TWITTER_CONSUMER_KEY` | No | - | Twitter API consumer key |
| `TWITTER_CONSUMER_SECRET` | No | - | Twitter API consumer secret |
| `TWITTER_ACCESS_TOKEN` | No | - | Twitter API access token |
| `TWITTER_ACCESS_TOKEN_SECRET` | No | - | Twitter API access token secret |
| `REDDIT_CLIENT_ID` | No | - | Reddit API client ID |
| `REDDIT_CLIENT_SECRET` | No | - | Reddit API client secret |
| `REDDIT_USER_AGENT` | No | `soundhash_bot_v1.0` | Reddit API user agent string |
| `REDDIT_REFRESH_TOKEN` | No | - | Reddit API refresh token |

#### OAuth and Authentication

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `CALLBACK_BASE_URL` | No | `http://localhost:8000` | Base URL for OAuth callbacks |
| `AUTH_SERVER_HOST` | No | `0.0.0.0` | OAuth server bind address |
| `AUTH_SERVER_PORT` | No | `8000` | OAuth server port |

#### Ingestion Settings

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `TARGET_CHANNELS` | No | - | Comma-separated list of YouTube channel IDs to ingest |
| `CHANNEL_RETRY_DELAY` | No | `5` | Seconds to wait between channel ingestion retries |
| `CHANNEL_MAX_RETRIES` | No | `3` | Maximum retry attempts for failed channel ingestion |

#### Bot Settings

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `BOT_NAME` | No | `@soundhash_bot` | Bot username/handle |
| `BOT_KEYWORDS` | No | `find clip,source video,original,what song` | Comma-separated trigger keywords |

**Configuration Notes:**
- All settings are loaded from `.env` file at startup via centralized `config/settings.py`
- Missing critical settings (database credentials) raise clear errors at startup
- Boolean values accept `true`/`false` (case-insensitive)
- List values (proxies, channels, keywords) are comma-separated strings
- No module should read `os.environ` directly - import from `config.settings.Config` instead
