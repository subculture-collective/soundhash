...
class VideoRepository:
    ...
    def get_channel_by_id(self, channel_id: str) -> Optional[Channel]:
        ...
        return (
            self.session.query(Channel).filter(Channel.channel_id == channel_id).first()
        )
    ...
    def mark_video_processed(
        self, video_id: int, success: bool = True, error_message: str = None
    ):
...
class JobRepository:
    ...
    def create_job(
        self, job_type: str, target_id: str, parameters: str = None
    ) -> ProcessingJob:
    ...
    def get_pending_jobs(
        self, job_type: str = None, limit: int = 10
    ) -> List[ProcessingJob]:
        """Get pending jobs"""
