...
class ChannelIngester:
        ...
        if not self.youtube_service and YOUTUBE_API_AVAILABLE:
            try:
                ...
                self.logger.log_warning_box(
                    f"Failed to initialize YouTube API service: {e}"
                )
        else:
            self.logger = create_section_logger(__name__)
    ...
    async def ingest_channel(
        self, channel_id: str, max_videos: int = None, dry_run: bool = False
    ):
        """
        ...
        try:
            ...
            videos_info = self.video_processor.get_channel_videos(
                    ...
                    if existing_video:
                        ...
                        if self._should_update_video(existing_video, video_info):
                            self._update_video_record(
                                existing_video, video_info, video_repo
                            )
                            updated_videos += 1
                    else:
...
class VideoJobProcessor:
    ...
    async def process_video_job(
        self, job, video_repo: VideoRepository, job_repo: JobRepository
    ):
        ...
        try:
            ...
            job_repo.update_job_status(
                job.id, "running", 0.2, "Downloading and segmenting audio"
            )
            ...
            job_repo.update_job_status(
                ...
                "running",
                0.5,
                f"Extracting fingerprints from {len(segments)} segments",
            )
            ...
            for i, (segment_file, start_time, end_time) in enumerate(segments):
                try:
                    # Extract fingerprint
                    fingerprint_data = self.fingerprinter.extract_fingerprint(
                        segment_file
                    )

                    # Store fingerprint in database
                    serialized_data = self.fingerprinter.serialize_fingerprint(
                        fingerprint_data
                    )
                    ...
                    self.logger.error(
                        f"Error processing segment {start_time}-{end_time}: {str(e)}"
                    )
                    continue
            ...
            if video:
                video_repo.mark_video_processed(
                    video.id, success=False, error_message=str(e)
                )
