...
class ChannelIngester:
    ...
    async def ingest_all_channels(
        self,
        channels_override: list | None = None,
        max_videos: int = None,
        dry_run: bool = False,
    ):
    ...
    def _parse_upload_date(self, upload_date_str: str) -> datetime | None:
        ...
        try:
            ...
            for i, (segment_file, start_time, end_time) in enumerate(segments):
                try:
                    ...
                    job_repo.update_job_status(
                        job.id, "running", progress, f"Processed segment {i+1}/{len(segments)}"
                    )
