...
class VideoProcessor:
    ...
    def __init__(
        self, temp_dir: str = "./temp", segment_length: int = None, youtube_service=None
    ):
        """
                ...
                for chrome_path in chrome_paths:
                    if os.path.exists(chrome_path):
                        ...
                        result = subprocess.run(
                            test_cmd, capture_output=True, timeout=5
                        )
            ...
            if result.stdout.strip():
                ...
                if len(parts) >= 11:
                    return {
                        ...
                        if parts[3] != "NA" and parts[3].isdigit()
                        else None,
                        "upload_date": parts[4] if parts[4] != "NA" else None,
                        "view_count": int(parts[5])
                        if parts[5] != "NA" and parts[5].isdigit()
                        else None,
                        "like_count": int(parts[6])
                        if parts[6] != "NA" and parts[6].isdigit()
                        else None,
    ...
    def download_video_audio(self, url: str, max_retries: int = 3) -> Optional[str]:
        """
            ...
            try:
                ...
                if Config.YT_PLAYER_CLIENT:
                    cmd.extend(
                        [
                            "--extractor-args",
                            f"youtube:player_client={Config.YT_PLAYER_CLIENT}",
                        ]
                    )
                ...
                if not actual_file or not actual_file.exists():
                    self.logger.error(
                        f"Downloaded audio file not found in: {self.temp_dir}"
                    )
                ...
                if "403" in str(e.stderr) or "Forbidden" in str(e.stderr):
                    if attempt < max_retries - 1:
                        self.logger.info(
                            "Got 403 error, will retry with different parameters..."
                        )
                        continue
    ...
    def segment_audio(
        ...
        try:
            ...
            while start_time < duration:
                ...
                segment_file = (
                    f"{self.temp_dir}/{base_name}_segment_{segment_id:04d}.wav"
                )
    ...
    def get_channel_videos(
        self, channel_id: str, max_results: int = None
    ) -> List[Dict]:
        """
        ...
        if self.youtube_service:
            try:
                ...
                self.logger.warning(
                    f"YouTube Data API failed for channel {channel_id}: {e}"
                )
    ...
    def process_video_for_fingerprinting(
        ...
        try:
            ...
            # should_cleanup_segments = (
            #     cleanup_segments
            #     if cleanup_segments is not None
            #     else Config.CLEANUP_SEGMENTS_AFTER_PROCESSING
            # )
            ...
            if (
                not Config.KEEP_ORIGINAL_AUDIO
                and audio_file
    ...
    def _download_video_info_ytdlp(self, url: str) -> Optional[Dict]:
    ...
    def _get_channel_videos_ytdlp(
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(video_ids)} video IDs for channel {channel_id}"
            )
            ...
            self.logger.error(
                f"Error getting videos for channel {channel_id}: {str(e)}"
            )
            return []
