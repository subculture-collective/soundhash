...
class AlertManager:
    ...
    def __init__(self):
        ...
        if self.enabled:
            logger.info("Alert manager enabled with thresholds: rate_limit=%d, job_failure=%d, window=%d min",
                       self.rate_limit_threshold, self.job_failure_threshold, 
    ...
    def record_rate_limit_failure(self, error_code: str, url: str, error_message: str) -> None:
        ...
        # Log the failure
        logger.warning(f"Rate limit failure recorded: HTTP {error_code} - {len(self.rate_limit_failures)} failures in window")
        
        # Check threshold
        if len(self.rate_limit_failures) >= self.rate_limit_threshold:
            if self._should_send_alert(self.last_rate_limit_alert):
                message = f"Rate Limit Threshold Exceeded ({len(self.rate_limit_failures)} failures)"
                details = (
                ...
                details += (
                    f"\n**Recommended actions:**\n"
                    f"• Check YouTube rate limits and quotas\n"
                    f"• Verify proxy configuration (USE_PROXY, PROXY_URL)\n"
                    f"• Enable cookie authentication (YT_COOKIES_FROM_BROWSER)\n"
                    f"• Reduce concurrent downloads (MAX_CONCURRENT_DOWNLOADS)\n"
    ...
    def record_job_failure(self, job_type: str, job_id: int, error_message: str) -> None:
        ...
        # Log the failure
        logger.warning(f"Job failure recorded: {job_type} job {job_id} - {len(self.job_failures)} failures in window")
        ...
        if len(self.job_failures) >= self.job_failure_threshold:
            if self._should_send_alert(self.last_job_failure_alert):
                ...
                details += (
                    f"\n**Recommended actions:**\n"
                    f"• Check application logs for error patterns\n"
                    f"• Verify database connectivity\n"
                    f"• Check available disk space in TEMP_DIR\n"
                    f"• Review recent configuration changes\n"
                    f"• See logs for full details"
                )
